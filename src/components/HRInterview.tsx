...
const HRInterview = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setModelsLoaded(true); // allow continuing even if local vision fails
      }
    };
  ...
  const startRecording = useCallback(() => {
    ...
    // ────────────────────────────────────────────────
    // AI vision analysis – now explicitly requesting Gemini 2.5 Pro
    // ────────────────────────────────────────────────
      ...
      try {
        ...
        const { data, error } = await supabase.functions.invoke('analyze-presentation', {
          body: {
            ...
            model: "gemini-2.5-pro"          // ← Explicitly Gemini 2.5 Pro
          }
        });
    ...
    setTimeout(runAnalysis, 500); // initial call

    // Local frame-by-frame analysis
    const analyzeFrame = async () => {
      ...
      try {
        ...
        if (visionMetrics) {
          ...
          // Blend with latest Gemini analysis if available
  ...
  // ────────────────────────────────────────────────
  // Answer evaluation – explicitly using Gemini 2.5 Pro
  // ────────────────────────────────────────────────
  ...
  const getEmotionBadgeColor = (emotion?: string) => {
    switch (emotion?.toLowerCase()) {
      ...
      case 'nervous':
      case 'anxious': return 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50';
      case 'stressed':
      case 'angry':
      case 'frustrated': return 'bg-destructive/20 text-destructive border-destructive/50';
      case 'focused':
      case 'attentive': return 'bg-purple-500/20 text-purple-400 border-purple-500/50';
      case 'calm':
      case 'neutral':
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        {!interviewStarted ? (
          ...
          <div className="max-w-3xl mx-auto">
            <Card className="p-8 bg-gradient-card border-border text-center">
              ...
              <p className="text-muted-foreground mb-6">
                Practice answering behavioral questions with real-time AI feedback powered by Gemini 2.5 Pro.
              ...
              <div className="bg-secondary/30 rounded-lg p-4 mb-6">
                ...
                <ul className="text-sm text-muted-foreground space-y-1 text-left max-w-md mx-auto">
                  ...
                  <li>• Gemini 2.5 Pro analyzes video for emotion, posture & gestures</li>
                  <li>• Real-time speech transcription</li>
                  <li>• STAR method evaluation after each answer</li>
                  <li>• Detailed performance summary</li>
                </ul>
              </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            {/* Left column – Question + Video + Controls */}
                ...
                <Card className="p-4 bg-gradient-card border-border">
                  {isEvaluating ? (
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-muted-foreground">Evaluating with Gemini 2.5 Pro...</span>
                    </div>
                  ) : (
                    <div>
