...
const HRInterview = () => {
  ...
  const aiVisionMetricsRef = useRef(aiVisionMetrics);
  ...
  useEffect(() => { aiVisionMetricsRef.current = aiVisionMetrics; }, [aiVisionMetrics]);
  ...
  const toggleMicrophone = () => {
   ...
   const startRecording = useCallback(() => {
     ...
     const runAnalysis = async () => {
       ...
       try {
         ...
         // FIX: Use refs instead of stale state
         const { data, error } = await supabase.functions.invoke('analyze-presentation', {
           body: { imageData, transcript: transcriptRef.current + ' ' + interimTranscriptRef.current }
         });
   ...
   const stopRecording = useCallback(async () => {
     ...
     // FIX: Use refs to get current values instead of stale closure state
     const currentTranscript = transcriptRef.current;
     const currentAiVisionMetrics = aiVisionMetricsRef.current;
     ...
     try {
        const { data, error } = await supabase.functions.invoke('evaluate-hr-answer', {
          body: {
            ...
            visionMetrics: {
              ...avgMetrics,
              detectedEmotion: currentAiVisionMetrics?.detectedEmotion,
              gestureType: currentAiVisionMetrics?.gestureType,
              postureType: currentAiVisionMetrics?.postureType,
            },
                ...
                <Card className="p-4 bg-gradient-card border-border">
                  {isEvaluating ? (
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-muted-foreground">Evaluating your answer with AI...</span>
                    </div>
                  ) : (
                    <div>
